...
class SystemPredictor(object):
    ...
    def predict_topk(self, img):
        ...
        for idx, result in enumerate(results):
            ...
            # store top_k
            for i in range(self.return_k):
                preds["rec_docs"] = self.id_map[docs[0][i]].split()[1]
                preds["rec_scores"] = scores[0][i]
                output2[i].append(preds.copy())
        ...
        output_topk = []
        for i in range(self.return_k):
            output_topk += self.nms_to_rec_results(output2[i], self.config["Global"]["rec_nms_thresold"])
...
def main(config):
    ...
    for idx, image_file in enumerate(image_list):
        ...
        output = system_predictor.predict_topk(img)
