...
# Move net and img to gpu
net.collect_params().reset_ctx(ctx)
img_ctx = img.expand_dims(axis=0)
img_ctx = img_ctx.as_in_context(ctx)

# warming up network for a fair comparision
net(img_ctx)
...
for i in range(run_cnt):
    pred = net(img_ctx)
toc = time.time()
inference_time = (toc - tic) / float(run_cnt) * 1000 # ms
print("Inference time: {:10.6f} ms".format(inference_time))
